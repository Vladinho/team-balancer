...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (saved) {
            try { setPlayers(JSON.parse(saved)); } catch { setPlayers(initialPlayers); }
        } else {
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        const exists = players.some(p => p.name.toLowerCase().trim() === formData.name.toLowerCase().trim() && (!editingPlayer || p.id !== editingPlayer.id));
        if (exists) return;
        if (editingPlayer) {
        ...
        <Container className="py-4 text-light bg-dark min-vh-100">
            <div className="mx-auto" style={{ maxWidth: 900 }}>
                ...
                <Row className="mb-4 align-items-center justify-content-center gap-3">
                    <Col xs="auto">
                        <Button variant="success" onClick={openAddModal}>+ Добавить игрока</Button>
                    </Col>
                    <Col xs="auto" className="d-flex align-items-center">
                        ...
                        <Button className="ms-3" onClick={splitTeams} disabled={selected.length < teamsCount}>Разделить</Button>
                    </Col>
                </Row>
                                ...
                                <Col key={idx} xs={12} md={6} className="mb-3">
                                    <div className="bg-secondary p-3 rounded text-light">
                                        ...
                                        <ul className="list-unstyled mb-0">
                                            {team.map(p => (
                                                <li key={p.id}>{p.name} {p.nickname && `(${p.nickname})`}</li>
                                            ))}
                                        </ul>
                                    </div>
                                </Col>
                ...
                {players.length && <Table bordered hover variant="dark" responsive className="text-center">
                    ...
                    {players.map((p, i) => (
                        <tr key={p.id} className={selected.includes(p.id) ? "table-primary" : ""}>
                            <td><Form.Check checked={selected.includes(p.id)} onChange={() => handleSelect(p.id)} /></td>
                            <td>{i + 1}</td>
                            <td>{p.name}</td>
                            <td>{p.nickname || '-'}</td>
                            <td>{p.rating}</td>
                            <td>
                                <Dropdown>
                                    <Dropdown.Toggle variant="outline-light" size="sm">⋮</Dropdown.Toggle>
                                    <Dropdown.Menu >
                                        <Dropdown.Item onClick={() => openEditModal(p)}>Редактировать</Dropdown.Item>
                                        <Dropdown.Item onClick={() => deletePlayer(p.id)} className="text-danger">Удалить</Dropdown.Item>
                                    </Dropdown.Menu>
                                </Dropdown>
                            </td>
                        </tr>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)} centered>
                ...
                <Form onSubmit={handleFormSubmit}>
                    <Modal.Body>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label>Ник <span className="text-secondary">(опционально)</span></Form.Label>
                            <Form.Control
                                type="text"
                    ...
                    <Modal.Footer>
                        ...
                        <Button variant="primary" type="submit">{editingPlayer ? 'Сохранить' : 'Добавить'}</Button>
                    </Modal.Footer>
                </Form>
            </Modal>
        </Container>
