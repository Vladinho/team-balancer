...
export const App: React.FC = () => {
    ...
    const handleSplit = () => split(players, selected, teamsCount);
    const openAdd = () => { setFormData({ name: '', nickname: '', rating: 5 }); open(null); };
    const openEdit = (p: Player) => { setFormData({ name: p.name, nickname: p.nickname, rating: p.rating }); open(p); };
        ...
        <Container className="py-4 text-light bg-dark min-vh-100">
            <div className="mx-auto" style={{ maxWidth: 900 }}>
                ...
                <Row className="mb-4 align-items-center justify-content-center gap-3">
                    <Col xs="auto">
                        <Button variant="success" onClick={openAdd}>+ Добавить игрока</Button>
                    </Col>
                    <Col xs="auto" className="d-flex align-items-center">
                        ...
                        <Button className="ms-3" onClick={handleSplit} disabled={selected.length < teamsCount}>
                            Разделить
                        </Button>
                    </Col>
                </Row>
